...
const multiPalEmailHandlerFun  = (response,response2,response3)=>{
...
router.get('/all', async function (req, res) {
    ...
    return res.status(200).json(multiPalEmailHandlerFun(response,response2,response3));
    } catch (err) {
...
router.get('/filter-by-employee/:employeeId', async function (req, res) {
  ...
  try {
      ...
      const response3 = await prospects.find({employeeMongoId: employeeId})
...
router.get('/filter-by-admin/:partnerAdminId', async function (req, res) {
  ...
  try {
      ...
      const response3 = await prospects.find({partnerAdminMongoId: partnerAdminId})
